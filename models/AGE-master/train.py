...
def gae_for(args):
    ...
    pos_inds_cuda = torch.LongTensor(pos_inds).cpu()
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        while ( ed <= length ):
            ...
            sampled_neg = torch.LongTensor(np.random.choice(neg_inds, size=ed-st)).cpu()
            ...
            batch_label = torch.cat((torch.ones(ed-st), torch.zeros(ed-st))).cpu()
