...
def feature_weight_threshold_estimation(graph_features,labelled_file):
    ...
    for train_index, test_index in kf.split(X):
        ...
        report = classification_report(y_test, y_pred, target_names=['Anomaly 0', 'Anomaly 1'], output_dict=True)
        #print(report)
        precision_scores_0.append(report['Anomaly 0']['precision'])
        recall_scores_0.append(report['Anomaly 0']['recall'])
        f1_scores_0.append(report['Anomaly 0']['f1-score'])
        precision_scores.append(report['Anomaly 1']['precision'])
        recall_scores.append(report['Anomaly 1']['recall'])
        f1_scores.append(report['Anomaly 1']['f1-score'])  
    ...
    #    'Precision_0': precision_scores_0,  # Keep Node_IDs for test set
    #    'Recall_0': recall_scores_0,
    #    'F1-score_0': f1_scores_0,
    #    'Precision_1': precision_scores,  # Keep Node_IDs for test set
    #    'Recall_1': recall_scores,
    #    'F1-score_1': f1_scores
    #})
    #print(predicted_Df)
    #output = output + '_prediction.csv'
    #predicted_Df.to_csv(output)
    #print(np.mean(accuracy_scores), np.mean(recall_scores), np.mean(f1_scores))
    return np.mean(precision_scores_0), np.mean(recall_scores_0), np.mean(f1_scores_0),np.mean(precision_scores), np.mean(recall_scores), np.mean(f1_scores),np.mean(accuracy_scores)
